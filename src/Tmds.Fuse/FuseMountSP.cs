...
namespace Tmds.Fuse
{
    class FuseMountSP : IFuseMount
    {
        ...
        public unsafe FuseMountSP(string mountPoint, IFuseFileSystemSP fileSystem, MountOptions options, Encoding pathEncoding = null)
        {
            _mountPoint = mountPoint;
            _mountOptions = options;
            _pathEncoding = pathEncoding??Encoding.UTF8;
        ...
        private unsafe int Fallocate(path* path, int mode, ulong offset, ulong length, fuse_file_info* fi)
        {
            try
            {
                return _fileSystem.FAllocate(ToString(path), mode, offset, (long)length, ref ToFileInfoRef(fi));
            }
            catch
            {
                return -EIO;
        ...
        private unsafe int Access(path* path, mode_t mode)
        {
            try
            {
                return _fileSystem.Access(ToString(path), mode);
            }
            catch
            {
                return -EIO;
        ...
        private unsafe int Releasedir(path* path, fuse_file_info* fi)
        {
            try
            {
                return _fileSystem.ReleaseDir(ToString(path), ref ToFileInfoRef(fi));
            }
            catch
            {
                return -EIO;
        ...
        private unsafe int Opendir(path* path, fuse_file_info* fi)
        {
            try
            {
                return _fileSystem.OpenDir(ToString(path), ref ToFileInfoRef(fi));
            }
            catch
            {
                return -EIO;
        ...
        private unsafe int Removexattr(path* path, void* name)
        {
            try
            {
                return _fileSystem.RemoveXAttr(ToString(path), ToString((path*)name));
            }
            catch
            {
                return -EIO;
        ...
        private unsafe int Listxattr(path* path, void* buffer, UIntPtr size)
        {
            try
            {
                return _fileSystem.ListXAttr(ToString(path), new Span<byte>(buffer, (int)size));
            }
            catch
            {
                return -EIO;
        ...
        private unsafe int Getxattr(path* path, void* name, void* buffer, UIntPtr size)
        {
            try
            {
                return _fileSystem.GetXAttr(ToString(path), ToString((path*)name), new Span<byte>(buffer, (int)size));
            }
            catch
            {
                return -EIO;
        ...
        private unsafe int Setxattr(path* path, void* name, void* buffer, UIntPtr size, int flags)
        {
            try
            {
                return _fileSystem.SetXAttr(ToString(path), ToString((path*)name), new ReadOnlySpan<byte>(buffer, (int)size), flags);
            }
            catch
            {
                return -EIO;
        ...
        private unsafe int Fsync(path* path, fuse_file_info* fi)
        {
            try
            {
                return _fileSystem.FSync(ToString(path), ref ToFileInfoRef(fi));
            }
            catch
            {
                return -EIO;
        ...
        private unsafe int Flush(path* path, fuse_file_info* fi)
        {
            try
            {
                return _fileSystem.Flush(ToString(path), ref ToFileInfoRef(fi));
            }
            catch
            {
                return -EIO;
        ...
        private unsafe int Statfs(path* path, statvfs* vfs)
        {
            try
            {
                ...
                span.Clear();
                return _fileSystem.StatFS(ToString(path), ref MemoryMarshal.GetReference(span));
            }
            catch
            {
                return -EIO;
        ...
        private unsafe int Chown(path* path, uint uid, uint gid, fuse_file_info* fi)
        {
            try
            {
                return _fileSystem.Chown(ToString(path), uid, gid, ToFileInfo(fi));
            }
            catch
            {
                return -EIO;
        ...
        private unsafe int Rename(path* path, path* path2, int flags)
        {
            try
            {
                return _fileSystem.Rename(ToString(path), ToString(path2), flags);
            }
            catch
            {
                return -EIO;
        ...
        private unsafe int Symlink(path* path, path* path2)
        {
            try
            {
                return _fileSystem.SymLink(ToString(path2), ToString(path));
            }
            catch
            {
                return -EIO;
        ...
        private unsafe int Readlink(path* path, void* buffer, UIntPtr size)
        {
            try
            {
                return _fileSystem.ReadLink(ToString(path), new Span<byte>(buffer, (int)size));
            }
            catch
            {
                return -EIO;
        ...
        private unsafe int Utimens(path* path, timespec* tv, fuse_file_info* fi)
        {
            try
            {
                ...
                return _fileSystem.UpdateTimestamps(ToString(path),
        ...
        private unsafe int Link(path* fromPath, path* toPath)
        {
            try
            {
                return _fileSystem.Link(ToString(fromPath), ToString(toPath));
            }
            catch
            {
                return -EIO;
        ...
        private unsafe int Chmod(path* path, mode_t mode, fuse_file_info* fi)
        {
            try
            {
                return _fileSystem.ChMod(ToString(path), mode, ToFileInfo(fi));
            }
            catch
            {
                return -EIO;
        ...
        private unsafe int Truncate(path* path, ulong length, fuse_file_info* fi)
        {
            try
            {
                return _fileSystem.Truncate(ToString(path), length, ToFileInfo(fi));
            }
            catch
            {
                return -EIO;
        ...
        private unsafe int Create(path* path, mode_t mode, fuse_file_info* fi)
        {
            try
            {
                return _fileSystem.Create(ToString(path), mode, ref ToFileInfoRef(fi));
            }
            catch
            {
                return -EIO;
        ...
        private unsafe int Mkdir(path* path, mode_t mode)
        {
            try
            {
                return _fileSystem.MkDir(ToString(path), mode);
            }
            catch
            {
                return -EIO;
        ...
        private unsafe int Rmdir(path* path)
        {
            try
            {
                return _fileSystem.RmDir(ToString(path));
            }
            catch
            {
                return -EIO;
        ...
        private unsafe int Unlink(path* path)
        {
            try
            {
                return _fileSystem.Unlink(ToString(path));
            }
            catch
            {
                return -EIO;
        ...
        private unsafe int Write(path* path, void* buffer, UIntPtr size, ulong off, fuse_file_info* fi)
        {
            try
            {
                ...
                return _fileSystem.Write(ToString(path), off, new ReadOnlySpan<byte>(buffer, (int)size), ref ToFileInfoRef(fi));
            }
            catch
            {
                return -EIO;
        ...
        private unsafe int Getattr(path* path, stat* stat, fuse_file_info* fi)
        {
            try
            {
                ...
                span.Clear();
                return _fileSystem.GetAttr(ToString(path), ref MemoryMarshal.GetReference(span), ToFileInfo(fi));
            }
            catch
            {
                return -EIO;
        ...
        private unsafe int Readdir(path* path, void* buf, fuse_fill_dir* filler, ulong offset, fuse_file_info* fi, int flags)
        {
            try
            {
                ...
                return _fileSystem.ReadDir(ToString(path), offset, (ReadDirFlags)flags, ToDirectoryContent(buf, fillDelegate), ref ToFileInfoRef(fi));
            }
            catch
            {
                return -EIO;
        ...
        private unsafe int Open(path* path, fuse_file_info* fi)
        {
            try
            {
                return _fileSystem.Open(ToString(path), ref ToFileInfoRef(fi));
            }
            catch
            {
                return -EIO;
        ...
        private unsafe int Read(path* path, void* buffer, UIntPtr size, ulong off, fuse_file_info* fi)
        {
            ...
            {
                // TODO: handle size > int.MaxValue
                return _fileSystem.Read(ToString(path), off, new Span<byte>(buffer, (int)size), ref ToFileInfoRef(fi));
            }
            catch
            {
                return -EIO;
        ...
        private unsafe int Release(path* path, fuse_file_info* fi)
        {
            try
            {
                _fileSystem.Release(ToString(path), ref ToFileInfoRef(fi));
                return 0;
            }
            catch
            {
        ...
        private unsafe string ToString(path* path)
        {
            var span = new ReadOnlySpan<byte>(path, int.MaxValue);

            return _pathEncoding.GetString(span.Slice(0, span.IndexOf((byte)0)));
        }
